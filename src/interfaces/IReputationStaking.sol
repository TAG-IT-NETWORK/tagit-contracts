 ...
 *
 * Lifecycle:
 *   1. Registrant calls AgentIdentity.register() → agent created as INACTIVE
 *   2. Registrant calls stake(agentId, amount) with TAGIT tokens
 *   3. Registrant calls AgentIdentity.activate(agentId) → checks hasMinBond, sets ACTIVE
 *   4. If agent misbehaves, owner calls slash(agentId, amount)
 *   5. After decommission, registrant calls unstake(agentId) to reclaim bond
 */
